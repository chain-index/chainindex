...
func IsValidBlock(block *ci_pb.BcBlock) (bool, error) {
	if block == nil {
...
func TheBlockChainFingerprintMatchGenesisBlock(block *ci_pb.BcBlock) bool {
	return block.GetBlockchainFingerprintsRoot() == genesis.BLOCKCHAIN_FINGERPRINTS_ROOT
}

func NumberOfBlockchainsNeededMatchesChildBlock(block *ci_pb.BcBlock) bool {
...
func IfMoreThanOneHeaderPerBlockchainAreTheyOrdered(block *ci_pb.BcBlock) bool {
	headers := block.GetBlockchainHeaders()
...
func IsChainRootCorrectlyCalculated(block *ci_pb.BcBlock) bool {
	const (
...
func IsFieldLengthBounded(block *ci_pb.BcBlock) bool {
	return true
}

func IsMerkleRootCorrectlyCalculated(block *ci_pb.BcBlock) bool {
...
func IsDistanceAboveDifficulty(block *ci_pb.BcBlock) bool {
...
func IsDistanceCorrectlyCalculated(block *ci_pb.BcBlock) bool {
...
func IsValidBlockTime(block *ci_pb.BcBlock) bool {
	const (
...
func timeSortedBlockHeaders(headers *ci_pb.BlockchainHeaders) []*ci_pb.BlockchainHeader {
	out := make([]*ci_pb.BlockchainHeader, 0)
...
func prepareWork(block *ci_pb.BcBlock) string {
...
func headerListIsOrderedByHeight(hlist []*ci_pb.BlockchainHeader) bool {
	n := len(hlist)
	for i := n - 1; i > 0; i-- {
...
func markedTxHash(mtx *ci_pb.MarkedTransaction) string {
	asJSString := []string{}
...
func blockHeaderHash(header *ci_pb.BlockchainHeader, height uint64) string {
	const (
...
func getChildrenBlocksHashes(headers *ci_pb.BlockchainHeaders, height uint64) []string {
	out := []string{}
...
func isSkippableBlock(block *ci_pb.BcBlock) bool {
