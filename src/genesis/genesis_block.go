...
func createGenesisTransaction(miner string, balance *big.Int) (*ci_pb.TransactionOutput, error) {
	genTxOutput := new(ci_pb.TransactionOutput)
...
func BuildGenesisBlock(workDir string) (*ci_pb.BcBlock, error) {
	gStartingBalancesPath := filepath.Join(workDir, "data", "genesis", "at_genesis_balances.csv")
	gblock := new(ci_pb.BcBlock)
	...
	genTxs := make([]*ci_pb.Transaction, 0, 1)
	genTx := new(ci_pb.Transaction)
	// there are no utxos to gather for the genesis block
	genTx.Inputs = make([]*ci_pb.TransactionInput, 0)
	genTx.NinCount = 0
	// other boilerplate
	//genTx.Nonce = MINER
	genTx.Version = 1
	...
	genTxOutputs := make([]*ci_pb.TransactionOutput, 0)
	...
	gblock.BlockchainHeaders = new(ci_pb.BlockchainHeaders)
