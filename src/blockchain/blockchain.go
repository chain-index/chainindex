...
import (
	...
	"github.com/chain-index/chainindex/src/coin"
	"github.com/chain-index/chainindex/src/common"
	"github.com/chain-index/chainindex/src/database"
...
func DefaultChainindexBlockchainConfig() ChainindexBlockchainConfig {
	return ChainindexBlockchainConfig{
...
type ChainindexBlockchain struct {
	...
	currentHighestBlock              *ci_pb.BcBlock
...
func (obc *ChainindexBlockchain) connectToChildBlock(from, to dagger.Node) error {
...
func (obc *ChainindexBlockchain) AddBlock(block *ci_pb.BcBlock) bool {
	...
	if found {
		zap.S().Debugf(
			...
			common.BriefHash(parentNode.Attributes["block"].(*ci_pb.BcBlock).GetHash()),
			found,
		)
		obc.GraphMu.Lock()
	...
	for h, _ := range heads {
		...
		headBlock := headNode.Attributes["block"].(*ci_pb.BcBlock)
		...
		parentNode, found := obc.BlockGraph.GetNode(
			dagger.Path{
				XID:   headNode.Attributes["block"].(*ci_pb.BcBlock).GetPreviousHash(),
				XType: BLOCK_TYPE,
			})
		obc.GraphMu.Unlock()
		if found {
	...
	for {
		poppedHeadsMap := make(map[string]*ci_pb.BcBlock)
		poppedHeads := make([]*ci_pb.BcBlock, 0)
		if !obc.DB.IsInitialBlockDownload() {
		...
		for hash, _ := range heads {
			...
			highestBlock := headNode.Attributes["block"].(*ci_pb.BcBlock)
			obc.GraphMu.Lock()
			obc.BlockGraph.DFS(CONNECTION_TYPE, headNode.Path, func(node dagger.Node) bool {
				blk := node.Attributes["block"].(*ci_pb.BcBlock)
...
func (obc *ChainindexBlockchain) GetBlockByHash(hash string) (*ci_pb.BcBlock, error) {
	...
	obc.GraphMu.Unlock()
	if found {
		block = node.Attributes["block"].(*ci_pb.BcBlock)
	} else {
...
func (obc *ChainindexBlockchain) SyncProgress() ProgressInfo {
	...
	var currentBlock *ci_pb.BcBlock = nil
