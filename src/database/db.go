...
func orderedPairPrint(first, second *ci_pb.BcBlock) {
	zap.S().Debugf(
...
func orderedPairCheck(first, second *ci_pb.BcBlock) {
...
type ChainindexDB struct {
	...
	syncStartingBlock      *ci_pb.BcBlock   // the block we started syncing from in this run of chainindex
	tipOfSerializedChain   *ci_pb.BcBlock   // the highest, main-chain serialized block
	highestBlock           *ci_pb.BcBlock   // the highest block awaiting serialization
	toSerialize            []*ci_pb.BcBlock // sorted ascending in block height
	incomingBlocks         map[string]*ci_pb.BcBlock
	incomingBlocksByHeight map[uint64]string // map of height -> hash
	incomingBlocksByTx     map[string]string // map of tx hash -> hash
	txMemPool              map[string]*ci_pb.Transaction
	mtxMemPool             map[string]*ci_pb.MarkedTransaction
...
func (odb *ChainindexDB) Open(filepath string, dropChainstate bool, pruneDatabaseTo uint64) error {
	...
	odb.mu.Lock()
	odb.toSerialize = make([]*ci_pb.BcBlock, 0, 10*odb.Config.AncientChunkSize)
	odb.incomingBlocks = make(map[string]*ci_pb.BcBlock)
	odb.incomingBlocksByHeight = make(map[uint64]string)
	odb.incomingBlocksByTx = make(map[string]string)
	odb.txMu.Lock()
	odb.txMemPool = make(map[string]*ci_pb.Transaction)
	odb.mtxMemPool = make(map[string]*ci_pb.MarkedTransaction)
	odb.txMu.Unlock()
	...
	err = odb.db.View(func(tx *bolt.Tx) error {
		...
		if blocks == nil {
			tx.CreateBucketIfNotExists([]byte("CHAININDEX-BLOCK-CHUNKS"))
			tx.CreateBucketIfNotExists([]byte("CHAININDEX-BLOCK-CHUNK-MAP"))
			tx.CreateBucketIfNotExists([]byte("CHAININDEX-BLOCK-HEIGHT-TO-HASH"))
			tx.CreateBucketIfNotExists([]byte("CHAININDEX-TX-TO-BLOCK"))
		...
		for hash, sermtx := curmtx.First(); hash != nil; hash, sermtx = curmtx.Next() {
			strhash := hex.EncodeToString(hash)
			mtx := new(ci_pb.MarkedTransaction)
...
func (odb *ChainindexDB) Close() {
	...
	odb.toSerialize = make([]*ci_pb.BcBlock, 0, 0)
	odb.incomingBlocks = make(map[string]*ci_pb.BcBlock)
	odb.txMemPool = make(map[string]*ci_pb.Transaction)
	odb.mtxMemPool = make(map[string]*ci_pb.MarkedTransaction)
	odb.txMu.Unlock()
...
func (odb *ChainindexDB) addBlockUnsafe(block *ci_pb.BcBlock) bool {
...
func (odb *ChainindexDB) GetBlockByHeight(blockHeight uint64) (*ci_pb.BcBlock, error) {
	...
	block, ok2 := tryblock.(*ci_pb.BcBlock)
	var err error = nil
	if !(ok1 && ok2) {
		...
		if !ok1 {
			...
			err := odb.db.View(func(tx *bolt.Tx) error {
				height2hash := tx.Bucket(ChainindexHeightToHashMap)
...
func (odb *ChainindexDB) GetBlockByTx(txHash []byte) (*ci_pb.BcBlock, error) {
	...
	if !(ok1 && ok2) {
		...
		if !ok1 {
			...
			err := odb.db.View(func(tx *bolt.Tx) error {
				tx2hash := tx.Bucket(ChainindexTxToHashMap)
...
func (odb *ChainindexDB) getSerializedBlock(blockHash []byte) (*ci_pb.BcBlock, error) {
	blockHashString := hex.EncodeToString(blockHash)
	block := new(ci_pb.BcBlock)
	ok := false

	err := odb.db.View(func(tx *bolt.Tx) error {
		chunks := tx.Bucket([]byte("CHAININDEX-BLOCK-CHUNKS"))
		block2chunk := tx.Bucket([]byte("CHAININDEX-BLOCK-CHUNK-MAP"))

		if block2chunk == nil {
			return errors.New("CHAININDEX-BLOCK-CHUNK-MAP not initialized in database!")
		}
...
func (odb *ChainindexDB) FullLocalValidation() {
	...
	err := odb.db.View(func(tx *bolt.Tx) error {
		heights := tx.Bucket(ChainindexHeightToHashMap)
		chunks := tx.Bucket(ChainindexBlockChunks)
		block2chunk := tx.Bucket(ChainindexBlockChunkMap)
		...
		blockMap := make(map[string]*ci_pb.BcBlock)
		for k, v := c.Seek(seekBytes); k != nil; k, v = c.Next() {
			...
			if bytes.Compare(currentChunk, chunkHash) != 0 {
				blockMap = make(map[string]*ci_pb.BcBlock) // reset the blockmap
...
func (odb *ChainindexDB) runSerialization() {
...
func (odb *ChainindexDB) serializeBlocks(inblocks []*ci_pb.BcBlock) error {
	c := &lz4.CompressorHC{}
	blocks := ci_pb.BcBlocks{Blocks: inblocks}
	...
	return odb.db.Batch(func(tx *bolt.Tx) error {
		...
		chunks, err := tx.CreateBucketIfNotExists([]byte("CHAININDEX-BLOCK-CHUNKS"))
		if err != nil {
			return err
		}
		// block hashes to chunks that store that block
		block2chunk, err := tx.CreateBucketIfNotExists([]byte("CHAININDEX-BLOCK-CHUNK-MAP"))
		if err != nil {
			return err
		}
		// block height to block hash
		height2hash, err := tx.CreateBucketIfNotExists([]byte("CHAININDEX-BLOCK-HEIGHT-TO-HASH"))
		if err != nil {
			return err
		}
		tx2hash, err := tx.CreateBucketIfNotExists([]byte("CHAININDEX-TX-TO-BLOCK"))
		if err != nil {
			return err
		}
...
func (odb *ChainindexDB) deleteBlocksAfter(pruneChainTo uint64) error {
	for hash, block := range odb.incomingBlocks {
	...
	err := odb.db.Update(func(tx *bolt.Tx) error {
		blockChunks := tx.Bucket(ChainindexBlockChunks)
		hash2chunk := tx.Bucket(ChainindexBlockChunkMap)
		height2hash := tx.Bucket(ChainindexHeightToHashMap)
		tx2Hash := tx.Bucket(ChainindexTxToHashMap)
		syncInfo := tx.Bucket(SyncInfo)
